...
test('marketing navigation and interactive controls work', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: 'Choose the split' })).toBeVisible();

  const footer = page.locator('footer');
  await footer.getByRole('link', { name: 'Privacy' }).click();
  await expect(page.getByRole('heading', { name: /Your data, treated with care/i })).toBeVisible();
