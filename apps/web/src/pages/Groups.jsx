        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {data.groups.map((group) => (
            <Link key={group._id} to={`/app/groups/${group._id}`}>
              <Card className="group group-card h-full p-5">
                <div className="flex items-start justify-between">
                  <span className="group-card-mark grid size-11 place-items-center rounded-2xl bg-violet text-sm font-bold text-white">
                    {group.name.slice(0, 1).toUpperCase()}
                  </span>
                  <ArrowRight className="size-5 text-slate-400 transition group-hover:text-violet" />
                </div>
                <h2 className="mt-8 text-lg font-bold">{group.name}</h2>
                ...
                <div className="mt-5 flex items-center gap-2 text-xs font-semibold text-slate-500">
                  ...
                  <span className="ml-auto rounded-md bg-slate-100 px-2 py-1 dark:bg-white/10">
                    {group.currency}
                  </span>
                </div>
              </Card>
    ...
    <Modal open={open} onClose={onClose} title="New group">
      ...
      >
        ...
        <label className="block">
          ...
          <select name="currency" className="field" defaultValue="INR">
        ...
        <div className="flex justify-end gap-2 pt-2">
          ...
          <Button loading={mutation.isPending}>Create group</Button>
        </div>
      </form>
    </Modal>
  );
